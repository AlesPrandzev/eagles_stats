...
# 2. FILTROVÁNÍ (Sidebar)
st.sidebar.header("Nastavení")

# Získání seznamu hráčů bez "Neznámých" (pokud tam nějací z pickoffů zbyli)
hraci = sorted([h for h in df['Pálkař'].unique() if "Neznámý" not in h])
vybrany_hrac = st.sidebar.selectbox("Vyber pálkaře:", hraci)
...
# 3. ZOBRAZENÍ VÝSLEDKŮ PRO HRÁČE
...
with tab1:
    ...
    with col1:
        ...
        # Odkud dává hráč hity?
        st.subheader("Z jakého stavu dává hráč Hity?")
        df_hits = df_filt[df_filt['H'] == 1]
        hit_counts = df_hits['Count'].value_counts().reset_index()
        hit_counts.columns = ['Stav', 'Počet Hitů']
...
with tab2:
    st.write("Kompletní historie nadhozů pro vybraného hráče:")
    zobrazene_sloupce = ['Zápas_ID', 'Fáze', 'Směna', 'Outy', 'Count', 'Nadhazovač', 'Popis_Akce']
